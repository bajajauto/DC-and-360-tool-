import { ArrowLeft, Check, ChevronRight, Clock, Download, FileText, Mail, RotateCw, Users } from 'lucide-react'
...
export default function ParticipantDetail() {
  ...
  async function handleGenerateReport() {
    setReportAction({ loading: true, action: 'generate', error: '' })
    try {
      ...
      setParticipant((current) => {
        ...
        const reportStatus = current.reportStatus === 'released' ? 'released' : 'generated'
        return { ...current, reportStatus, progress: 100, taskStatus, taskCompletionPercent, lastActivity: new Date().toISOString() }
      })
    } catch (err) {
      setReportAction({ loading: false, action: null, error: err.message || 'Unable to generate the report.' })
      return
    }
    setReportAction({ loading: false, action: null, error: '' })
  }

  async function handlePublishReport() {
    setReportAction({ loading: true, action: 'publish', error: '' })
    try {
      ...
      setReportAction({ loading: false, action: null, error: err.message || 'Unable to publish the report.' })
      return
    }
    setReportAction({ loading: false, action: null, error: '' })
  }
    ...
    <div className="p-8 max-w-[1360px] mx-auto">
      ...
      <div className="grid xl:grid-cols-[1fr_320px] gap-6">
        <div className="space-y-6">
          ...
          <section className="bg-white border border-[#e2e8f0] rounded-2xl overflow-hidden">
            <div className="px-6 py-5 border-b border-[#e8edf3]">
              <h3 className="font-semibold text-[#172033]">Nominated respondents</h3>
              <p className="text-xs text-gray-400 mt-1">Names are visible for tracking completion; feedback remains confidential</p>
            </div>
        ...
        <aside className="space-y-5">
          <section className="rounded-2xl bg-[#173f72] p-5 text-white">
            ...
            {reportGenerated && <button onClick={handleGenerateReport} disabled={reportAction.loading} className="mt-5 flex w-full items-center justify-center gap-2 rounded-lg bg-amber-300 py-2.5 text-sm font-semibold text-[#12345a] disabled:opacity-60"><RotateCw size={15} />{reportAction.action === 'generate' ? 'Regenerating…' : 'Regenerate report'}</button>}
            {reportGenerated && !reportReleased && <button onClick={handlePublishReport} disabled={reportAction.loading} className="mt-3 w-full rounded-lg bg-emerald-400 py-2.5 text-sm font-semibold text-[#12345a] disabled:opacity-60">{reportAction.action === 'publish' ? 'Publishing…' : 'Publish report'}</button>}
