...
import BulkAssessorUpload from './BulkAssessorUpload'
...
export default function AssessorAnalysis() {
  ...
  async function upload() {
    if (!file || !selected || uploading || bulkBusy) return
    ...
    try {
      ...
      setMessage('Workbook uploaded successfully. Generating the DC report…')
      try {
        await api.generateDcReport(selected.participantId)
        setMessage('Workbook uploaded and the DC report generated successfully.')
      } catch (error) {
        setMessage(`Workbook uploaded successfully, but DC report generation failed: ${error.message}`)
      }
  ...
  return <div className="px-9 py-8"><div className="mx-auto max-w-[1400px]">
    ...
    <BulkAssessorUpload rows={rows} loading={loading} busy={uploading || bulkBusy} onBusyChange={setBulkBusy} onUploaded={(participantId, workbook) => setRows((current) => current.map((row) => row.participantId === participantId ? { ...row, workbook } : row))} />
