...
export default function BUHRDashboard() {
  const { user } = useUser()
    ...
    <div>
      <header className="h-20 bg-white border-b border-[#e4e9f1] px-8 flex items-center justify-between">
        <div>
          ...
          <h1 className="text-xl font-bold text-[#172033]">People in DC</h1>
        </div>
      </header>
      ...
      <div className="p-8 max-w-[1360px] mx-auto">
        ...
        <section className="mt-6 overflow-hidden rounded-xl border border-[#d5dce5] bg-white shadow-[0_2px_16px_rgba(31,41,55,.06)]">
          <div className="flex flex-col gap-3 border-b border-[#d5dce5] px-5 py-4 md:flex-row md:items-center md:justify-between">
            <div>
              <h2 className="text-[15px] font-semibold text-[#0f172a]">Current status</h2>
              <p className="mt-1 text-xs text-slate-500">Progress is visible throughout the process. Reports unlock only after TD publishes them.</p>
            </div>
            <div className="relative">
              <Search size={15} className="absolute left-3 top-2.5 text-slate-400" />
              <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search people" className="w-full rounded-lg border border-[#c2ccda] bg-white py-2 pl-9 pr-3 text-xs focus:outline-none focus:ring-2 focus:ring-[#d6e4f7] md:w-64" />
            </div>
          </div>
          ...
          {(loading || !filtered.length) && (
            <div className="py-14 text-center">
              ...
              <p className="mt-3 text-sm text-slate-500">{loading ? 'Loading BUHR view...' : 'No people match this search.'}</p>
            </div>
          )}
        </section>
      </div>
