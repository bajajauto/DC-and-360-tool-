...
export default function BUHRReportRepository() {
  ...
  const rows = useMemo(() => {
    ...
    const searched = scoped.filter((participant) => !normalized || `${participant.name} ${participant.employeeId} ${participant.designation} ${participant.bu} ${participant.cohort.name}`.toLowerCase().includes(normalized))
  ...
  return <div>
    <header className="h-20 border-b border-[#e4e9f1] bg-white px-8 flex items-center">
      <div><p className="mb-1 text-xs text-gray-400">BUHR / {payload?.businessUnit || user?.bu || 'Business Unit'}</p><h1 className="text-xl font-bold text-[#172033]">Report Repository</h1><p className="mt-1 text-xs text-slate-500">DC Reports and 360° Feedback Reports made available by TD for employees in your business unit.</p></div>
    </header>
    <div className="mx-auto max-w-[1360px] p-8">
      ...
      <section className="overflow-hidden rounded-2xl border border-[#d5dce5] bg-white">
        <div className="flex flex-col gap-4 border-b border-[#d5dce5] p-5 lg:flex-row lg:items-end lg:justify-between">
          <div><h2 className="text-lg font-bold text-[#1e5fba]">All reports</h2><div className="mt-3 inline-flex rounded-lg border border-[#dce3ed] bg-slate-50 p-1">{[['all', 'All reports'], ['dc', 'DC reports'], ['360', '360° Feedback Reports']].map(([value, label]) => <button key={value} onClick={() => setReportType(value)} className={`rounded-md px-3 py-1.5 text-xs font-semibold transition-colors ${reportType === value ? 'bg-[#1e5fba] text-white shadow-sm' : 'text-slate-600 hover:bg-white'}`}>{label}</button>)}</div></div>
