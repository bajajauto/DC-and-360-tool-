...
type Relationship = 'Self' | 'Reporting Manager' | 'Skip Manager' | 'Peer' | 'Direct Report'
...
const relationshipStyles: Record<Relationship, { row: string; avatar: string; badge: string }> = {
      ...
      <div className="bg-white rounded-xl border border-[#e2e8f0] overflow-hidden">
        ...
        <div className="space-y-2 p-3">
          {orderedRespondents.map((r) => (
            <div key={r.name} className={`flex items-center gap-4 rounded-lg border px-4 py-3.5 ${relationshipStyles[r.relationship].row}`}>
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-semibold shrink-0 ${relationshipStyles[r.relationship].avatar}`}>
                {initials(r.name)}
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-[#1a1f2e]">{r.name === 'Rahul Kumar (Self)' ? 'You (Self)' : r.name}</p>
                <span className={`inline-flex mt-0.5 text-[10px] font-medium px-1.5 py-0.5 rounded-full ${relationshipStyles[r.relationship].badge}`}>
                  {r.relationship}
                </span>
              </div>
