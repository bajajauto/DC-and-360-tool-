    ...
    <div className="min-h-screen bg-slate-100">
      <header className="flex h-20 items-center justify-between border-b border-[#e4e9f1] bg-white px-8">
        ...
        <button onClick={download} disabled={state.status === 'downloading'} className="flex items-center gap-2 rounded-lg bg-[#1e4d8c] px-4 py-2.5 text-xs font-semibold text-white hover:bg-[#173f72] disabled:opacity-50">
          <Download size={15} />
          {state.status === 'downloading' ? 'Preparing…' : 'Download PowerPoint'}
        </button>
      </header>

      <div className="mx-auto max-w-[1080px] px-8 py-4">
        {state.status === 'ready' && <div className="flex items-center gap-3 rounded-xl border border-blue-200 bg-blue-50 p-3.5"><Eye size={17} className="shrink-0 text-[#1e4d8c]" /><p className="text-xs text-blue-800">Preview the released report below or download the editable PowerPoint template.</p></div>}
