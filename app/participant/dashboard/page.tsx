...
const journeySteps = [
  { id: 1, label: "Role Interview", href: "/participant/role-interview", status: "completed", deadline: "15 Jun" },
  { id: 2, label: "Photograph", href: "/participant/photograph", status: "completed", deadline: "15 Jun" },
  { id: 3, label: "Pre-Work", href: "/participant/pre-work", status: "in-progress", deadline: "20 Jun" },
  { id: 4, label: "360 Nominees", href: "/participant/360-nominees", status: "pending", deadline: "20 Jun" },
  { id: 5, label: "360 Feedback", href: "/participant/360-status", status: "locked", deadline: "30 Jun" },
  { id: 6, label: "DC Report", href: "/participant/reports", status: "locked", deadline: "TBD" },
];
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span className={`text-[10px] font-medium px-2 py-0.5 rounded-full ${className}`}>{label}</span>
  );
}
            ...
            {journeySteps.map((step, idx) => (
              <div key={step.id} className="flex items-center gap-4 px-5 py-4">
                ...
                {(step.status === "in-progress" || step.status === "pending") && (
                  <Link
                    href={step.href}
                    className="shrink-0 text-xs text-[#1e4d8c] font-medium hover:underline"
                  >
                    Open →
                  </Link>
                )}
              ...
              {pendingTasks.map((task) => (
                ...
                >
                  ...
                  {task.progress > 0 && (
                    <div className="mb-3">
                      ...
                      <div className="w-full bg-gray-100 rounded-full h-1.5">
                        <div
                          className="bg-[#1e4d8c] rounded-full h-1.5"
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <Link
                      href={task.href}
                      className="text-xs text-[#1e4d8c] font-medium hover:underline"
                    >
                      Continue →
                    </Link>
                  </div>
