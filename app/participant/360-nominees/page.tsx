...
type Nominee = {
  ...
  email: string;
  designation?: string;
  relationship: "reporting-manager" | "skip-manager" | "peer" | "direct-report";
  source: "ec" | "external";
};

const ecDirectory = [
  { id: "e1", name: "Priya Menon", email: "priya.menon@bajaj.com", designation: "GM – Sales Strategy", relationship: "reporting-manager" as const },
  { id: "e2", name: "Vikram Sood", email: "vikram.sood@bajaj.com", designation: "VP – Operations", relationship: "skip-manager" as const },
...
const RELATIONSHIP_COLOR: Record<string, string> = {
  "reporting-manager": "bg-purple-100 text-purple-700",
  "skip-manager": "bg-indigo-100 text-indigo-700",
  peer: "bg-blue-100 text-blue-700",
  "direct-report": "bg-teal-100 text-teal-700",
};

function validate(nominees: Nominee[]): string[] {
  const errors: string[] = [];
  const managers = nominees.filter((n) => n.relationship === "reporting-manager");
  const peers = nominees.filter((n) => n.relationship === "peer");
  if (managers.length !== 1) errors.push("Exactly 1 Reporting Manager required.");
  if (peers.length < 4) errors.push(`At least 4 Peers / Internal Customers required (${peers.length} added).`);
  return errors;
}
...
export default function NomineesPage() {
  ...
  function addFromEC(person: typeof ecDirectory[0]) {
    if (nominees.find((n) => n.id === person.id)) return;
    setNominees((prev) => [
      ...prev,
      { ...person, relationship: "peer", source: "ec" as const },
    ]);
  }

  function addExternal() {
    if (!extEmail.trim() || !extName.trim()) return;
    setNominees((prev) => [
      ...prev,
      { id: `ext-${Date.now()}`, name: extName, email: extEmail, relationship: "peer", source: "external" as const },
    ]);
    setExtEmail("");
    setExtName("");
  }
  ...
  function changeRelationship(id: string, rel: Nominee["relationship"]) {
    setNominees((prev) => prev.map((n) => (n.id === id ? { ...n, relationship: rel } : n)));
  }

  const filtered = ecDirectory.filter(
    (p) =>
      !nominees.find((n) => n.id === p.id) &&
    ...
    <div className="p-8 max-w-4xl mx-auto">
      ...
      {submitted ? (
        <div className="bg-green-50 border border-green-200 rounded-xl p-8 text-center">
          ...
          <p className="text-xs text-green-600">360 invites will be dispatched to all nominees. Your BUHR can view this list.</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {/* Left: current nominees */}
          <div>
            <h2 className="text-sm font-semibold text-[#1a1f2e] mb-3 uppercase tracking-wide">
              Selected Nominees <span className="text-gray-400 font-normal normal-case">({nominees.length})</span>
            </h2>

            {/* Rules summary */}
            <div className="bg-[#f1f4f9] rounded-lg p-3 mb-4 space-y-1">
              {[
                { label: "Reporting Manager", required: "Exactly 1", count: grouped["reporting-manager"].length, ok: grouped["reporting-manager"].length === 1 },
                { label: "Skip Manager", required: "Optional, 1 max", count: grouped["skip-manager"].length, ok: true },
                { label: "Peers / Internal Customers", required: "Minimum 4", count: grouped.peer.length, ok: grouped.peer.length >= 4 },
                { label: "Direct Reports", required: "All (if any)", count: grouped["direct-report"].length, ok: true },
              ].map((r) => (
                <div key={r.label} className="flex items-center justify-between">
                  ...
                  <span className="text-xs text-gray-400">{r.count} · {r.required}</span>
                </div>
              ))}
            </div>

            {/* Grouped nominee list */}
            <div className="space-y-4">
              {(Object.keys(grouped) as Nominee["relationship"][]).map((rel) => (
                <div key={rel}>
                  <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-1.5">{RELATIONSHIP_LABELS[rel]}</p>
                  {grouped[rel].length === 0 ? (
                    <p className="text-xs text-gray-300 italic pl-1">None added</p>
                  ) : (
                    <div className="space-y-2">
                      {grouped[rel].map((n) => (
                        <div key={n.id} className="flex items-center gap-3 bg-white border border-[#e2e8f0] rounded-lg px-3 py-2.5">
                          <div className="w-7 h-7 rounded-full bg-[#dbeafe] flex items-center justify-center text-[#1e4d8c] text-xs font-semibold shrink-0">
                          ...
                          <select
                            ...
                            onChange={(e) => changeRelationship(n.id, e.target.value as Nominee["relationship"])}
                          ...
                          <button onClick={() => remove(n.id)} className="text-gray-300 hover:text-red-400 transition-colors ml-1">
          ...
          {/* Right: directory search + external */}
          <div className="space-y-5">
            ...
            <div className="bg-white rounded-xl border border-[#e2e8f0] p-5">
              ...
              <input
                ...
                className="w-full px-3 py-2 rounded-lg border border-[#e2e8f0] text-sm text-[#1a1f2e] placeholder-gray-300 focus:outline-none focus:ring-2 focus:ring-[#1e4d8c] focus:border-transparent mb-3"
              />
              <div className="space-y-1.5 max-h-64 overflow-y-auto">
                {filtered.map((p) => (
                  ...
                  >
                    ...
                    <svg className="w-4 h-4 text-[#1e4d8c] shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M12 4v16m8-8H4" />
                    </svg>
                  </button>
                ))}
                {filtered.length === 0 && (
                  <p className="text-xs text-gray-300 italic text-center py-4">No more employees to add</p>
                )}
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl border border-[#e2e8f0] p-5">
              ...
              <div className="space-y-2">
                <input
                  ...
                  className="w-full px-3 py-2 rounded-lg border border-[#e2e8f0] text-sm text-[#1a1f2e] placeholder-gray-300 focus:outline-none focus:ring-2 focus:ring-[#1e4d8c] focus:border-transparent"
                />
                <input
                  type="email"
