-- Re-run the targeted cleanup because this account was recreated after the original
-- deletion migration had already been recorded as applied in production.

DELETE FROM "EmailOutbox"
WHERE LOWER("toEmail") = 'dpaul1@bajajauto.co.in'
   OR "actorId" IN (SELECT "id" FROM "User" WHERE LOWER("email") = 'dpaul1@bajajauto.co.in')
   OR "magicLinkId" IN (
   ...
   OR ("entity" = 'FeedbackTask' AND "entityId" IN (
     SELECT ft."id" FROM "FeedbackTask" ft
